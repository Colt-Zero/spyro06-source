...
def highestPowerof2(n): 
  ...
  for i in range(n, 0, -1):
    ...
    if ((i & (i - 1)) == 0):
      res = i
      break
  res = int(math.log2(res))
  return res

def encode_image_from_path(new_image_file_path, image_format, palette_format, orig_width, orig_height, mipmap_count=1):
  image = Image.open(new_image_file_path)
  image_width, image_height = image.size
  pdw = highestPowerof2(orig_width) - highestPowerof2(image_width)
  pdh = highestPowerof2(orig_height) - highestPowerof2(image_height)
  #if abs(pdw) >= abs(pdh):
  #  pd = pdw
  #else:
  #  pd = pdh 
  ...
  return (new_image_data, new_palette_data, encoded_colors, image_width, image_height, mipmap_count)
