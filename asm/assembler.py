...
def get_bin(devkitbasepath, name):
  suff = ".exe"
...
class Assembler:
  def __init__(self, devkitpath):
    if platform == "win32":
      self.devkitbasepath = os.path.join(devkitpath, "bin")
    else:
      ...
      self.devkitbasepath = os.path.join(os.environ.get("DEVKITPPC"), "bin")
    print(self.devkitbasepath)
    if not os.path.isfile(get_bin(self.devkitbasepath, "powerpc-eabi-as")):
      raise Exception(r"Failed to assemble code: Could not find devkitPPC. Check if devkitPPC is installed to: %s" % self.devkitbasepath)
  ...
  def Assemble(self, dir=None):
    if getattr( sys, 'frozen', False ) :
      os.chdir(os.path.dirname(sys.executable))
    else:
      os.chdir(os.path.dirname(os.path.abspath(__file__)))
    cwd = os.getcwd()
    if os.path.exists(os.path.join(cwd, "asm")):
      os.chdir(os.path.join(cwd, "asm"))
    cwd = os.getcwd()
    if dir != None and os.path.exists(os.path.join(cwd, dir)):
      os.chdir(os.path.join(cwd, dir))
      
    print("Assembling %s patches" % dir)
      ...
      for filename in all_asm_files:
        ...
        for file_path, code_chunks_for_file in code_chunks.items():
          for org_offset_or_symbol, temp_asm in code_chunks_for_file.items():
            ...
            command = [
              get_bin(self.devkitbasepath, "powerpc-eabi-as"),
              "-mregnames",
              "-m750cl",
              temp_asm_name,
              "-o", o_name
            ...
            command = [
              get_bin(self.devkitbasepath, "powerpc-eabi-ld"),
              "-Ttext", "%X" % org_offset,
              "-T", temp_linker_name,
              "--oformat", "binary",
              "-Map=" + map_name,
...
#  if len(sys.argv) == 2 or platform != "win32":
#      assem = Assembler(sys.argv[1])
#      assem.Assemble()
#  else:
#    print(len(sys.argv))